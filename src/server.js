...
function llamarApiFF(uid, server = 'BR') {
  return new Promise((resolve, reject) => {
    ...
    req.on('error', (err) => { console.error(`[API FF] Error de conexión:`, err.message); reject(err); });
...
function interpretarRespuestaFF(apiData) {
  ...
  if (status === 1 || added > 0 || successful > 0) {
    if (after > 0 && before > 0 && after <= before) return { tipo: 'ya_recibio', data: apiData };
    if (after > 0 && before > 0 && after === before) return { tipo: 'ya_recibio', data: apiData };
...
app.get('/api/public-stats', async (req, res) => {
  try {
    ...
    res.json({ ok: true, usuarios: parseInt(usuarios.rows[0].count,10), likes: parseInt(likesTotal.rows[0].total,10), likes_hoy: parseInt(likesHoy.rows[0].total,10) });
  } catch (err) { res.json({ ok: false, usuarios: 0, likes: 0, likes_hoy: 0 }); }
});
...
app.get('/api/top-usuarios', async (req, res) => {
  ...
  } catch (err) { console.error(err); res.json({ ok: false, top: [] }); }
});
...
app.get('/api/perfil', authMiddleware, async (req, res) => {
  try {
    const today = new Date().toISOString().slice(0,10);
    await pool.query(
      `UPDATE usuarios SET envios_hoy=0, fecha_ultimo_envio=$1 WHERE id=$2 AND (fecha_ultimo_envio IS NULL OR fecha_ultimo_envio < $1)`,
      [today, req.user.id]
    );
    const result = await pool.query(
    ...
    const hist = await pool.query(
      `SELECT ff_uid,player_name,likes_antes,likes_despues,likes_agregados,nivel,region,fecha FROM historial WHERE usuario_id=$1 ORDER BY fecha DESC LIMIT 30`,
      [req.user.id]
    );
    const totalLikes = await pool.query(`SELECT COALESCE(SUM(likes_agregados),0) AS total FROM historial WHERE usuario_id=$1`, [req.user.id]);
...
app.post('/api/canjear', authMiddleware, async (req, res) => {
  try {
    ...
    if (cod.ilimitado) {
      await pool.query(
        `UPDATE usuarios SET plan_activo=true, plan_nombre='Plan Ilimitado', plan_tipo='ilimitado', likes_disponibles=999999, likes_limite_plan=999999, likes_enviados_plan=0, envios_por_dia=999, plan_vence=NULL, ilimitado=true WHERE id=$1`,
        [req.user.id]
      );
      res.json({ ok: true, message: '🚀 Plan Ilimitado activado correctamente' });
    } else if (cod.tipo === 'likes') {
      await pool.query(
        `UPDATE usuarios SET plan_activo=true, plan_nombre=$1, plan_tipo='likes', likes_disponibles=$2, likes_limite_plan=$2, likes_enviados_plan=0, envios_por_dia=$3, plan_vence=NULL, ilimitado=false WHERE id=$4`,
      ...
      res.json({ ok: true, message: `✅ Plan activado: ${cod.likes.toLocaleString()} likes · ${cod.envios_dia} envíos/día` });
    } else {
      const vence = new Date(Date.now() + cod.dias * 86400000).toISOString();
      await pool.query(
        `UPDATE usuarios SET plan_activo=true, plan_nombre=$1, plan_tipo='dias', likes_disponibles=0, likes_limite_plan=0, likes_enviados_plan=0, envios_por_dia=$2, plan_vence=$3, ilimitado=false WHERE id=$4`,
...
app.post('/api/enviar-likes', authMiddleware, async (req, res) => {
  try {
    ...
    // fromDiff = diferencia real visible en el perfil (más confiable)
    // Ejemplo: before=8106, after=8324 → fromDiff=218 → se muestra 218 exacto
    ...
    // Si fromDiff está en rango realista → usarlo directo (valor exacto real)
    // Si fromDiff es 0 o fuera de rango → usar el mayor de los otros campos
    let likesAdded = (fromDiff >= 210 && fromDiff <= 225)
      ? fromDiff
      : Math.max(fromAdded, fromSuccessful);

    // La API siempre envía mínimo 215; si reporta menos es error de los campos
    ...
    if (likesAdded > 0) {
      ...
      await pool.query(
        `INSERT INTO historial (usuario_id,ff_uid,player_name,likes_antes,likes_despues,likes_agregados,nivel,region) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
...
app.get('/api/admin/stats', adminMiddleware, async (req, res) => {
  try {
    ...
    res.json({
      ok: true,
      totalUsuarios: parseInt(tu.rows[0].count,10), totalCodigos: parseInt(tc.rows[0].count,10),
      codigosUsados: parseInt(uc.rows[0].count,10), planesActivos: parseInt(ap.rows[0].count,10),
      enviosHoy: parseInt(envHoy.rows[0].total,10), limiteApiDia: parseInt(process.env.FF_API_LIMIT_DIA || 0, 10),
...
app.put('/api/admin/usuarios/:id', adminMiddleware, async (req, res) => {
  try {
    ...
    const updated = await pool.query(
      `SELECT id,uid,username,contact,plan_activo,plan_nombre,plan_tipo,likes_disponibles,likes_limite_plan,likes_enviados_plan,envios_por_dia,envios_hoy,plan_vence,ilimitado,creado_en FROM usuarios WHERE id=$1`, [id]
    );
...
app.put('/api/admin/usuarios/:id/ilimitado', adminMiddleware, async (req, res) => {
  try {
    const id = req.params.id;
    await pool.query(
      `UPDATE usuarios SET plan_activo=true, plan_nombre='Plan Ilimitado', plan_tipo='ilimitado', likes_disponibles=999999, likes_limite_plan=999999, likes_enviados_plan=0, envios_por_dia=999, plan_vence=NULL, ilimitado=true WHERE id=$1`, [id]
    );
    const updated = await pool.query(
      `SELECT id,uid,username,contact,plan_activo,plan_nombre,plan_tipo,likes_disponibles,likes_limite_plan,likes_enviados_plan,envios_por_dia,envios_hoy,plan_vence,ilimitado,creado_en FROM usuarios WHERE id=$1`, [id]
    );
...
const PALABROTAS = [
  ...
  'estupido','estupidos','estupida','estupidas','estupido','estupida','retrasado','retrasada',
  'polla','pollas','joder','gilipollas','cabron','cabrones',
  ...
  'porra','caralho','foda','fodase','viado','buceta','boceta',
  'merda','otario','pau','cu','arrombado','cuzao',
  'pu7a','put4','sh1t','f4ck','a55','b1tch','v3rga','p3nd3jo',
];
...
app.get('/api/chat', authMiddleware, async (req, res) => {
  try {
    const r = await pool.query(
      `SELECT id, username, mensaje, creado_en FROM chat_mensajes WHERE creado_en > NOW() - INTERVAL '60 seconds' ORDER BY creado_en DESC LIMIT 20`
    );
...
app.post('/api/chat', authMiddleware, async (req, res) => {
  try {
    ...
    await pool.query(`INSERT INTO chat_mensajes (usuario_id, username, mensaje) VALUES ($1, $2, $3)`, [uid, user.rows[0].username, filtrado]);
    await pool.query(
      `DELETE FROM chat_mensajes WHERE id NOT IN (SELECT id FROM chat_mensajes ORDER BY creado_en DESC LIMIT 20) OR creado_en < NOW() - INTERVAL '60 seconds'`
    );
    res.json({ ok: true });
  } catch (err) { console.error(err); res.status(500).json({ error: 'Error interno' }); }
});
...
SOLO responde preguntas sobre la plataforma BoostSpeed. Si te preguntan algo ajeno responde amablemente que solo puedes ayudar con temas de BoostSpeed.
INFORMACIÓN:
...
- Hay 3 tipos de plan: por DÍAS, por LIKES, e ILIMITADO.
- Para enviar likes: ve a la pestaña "Likes", ingresa el UID de Free Fire y presiona Enviar.
- Solo puedes enviar likes a un mismo UID una vez cada 24 horas.
- Para canjear un código: ve a la pestaña "Acceso" e ingresa el código.
Responde de forma breve, amigable y clara. Máximo 3 oraciones.`;
    const body = JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 300, system: SYSTEM, messages: messages.slice(-8) });
    const options = {
  ...
  } catch (err) { console.error('AI chat error:', err); res.status(500).json({ error: 'Error interno del asistente' }); }
});
